...
$empName = '';
if (!empty($model)) {
    $empName =  $model->first_name. ' '. $model->last_name.' ('.$model->employee_code.')';
}
$this->title = 'Employee Record Logs';
$this->params['breadcrumbs'][] = ['label' => 'All Employees', 'url' => ['index']];
$this->params['breadcrumbs'][] = ['label' => $empName, 'url' => ['view', 'id' => $model->id]];
        ...
        <div class="profile-view">
            <ul class="updtlst">
                <?php if (count($data) > 0) { ?>
                    ...
                    echo "<p class='text-center'>No logs found</p>";
                } ?>
            </ul>
        </div>
    </div>
